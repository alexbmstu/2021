...
**Если устройство не было разблокировано, это означает, что один из пользователей не остановил ```dgb```. Вы можете сбросить карту командой ```sudo /opt/xilinx/overlaybins/reset.sh``` (даже несмотря на то, что у вы не являетесь членом группы sudoers!!!)**
